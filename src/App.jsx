...
function fmt(n) {
  ...
  return new Intl.NumberFormat("en-CA", { style:"currency", currency:"CAD", minimumFractionDigits:0, maximumFractionDigits:0 }).format(Math.abs(n));
}
function fmtFull(n) {
...
function parseCSV(text, accountId, merchantMemory={}) {
  ...
  const subIdx   = idx(["sub-description","subdescription","sub description","narrative"]);
  ...
  for(let i=1;i<lines.length;i++){
    ...
    rows.push({
      ...
      date:dateStr,month:monthKey,account:accountId,description,subDescription:subDesc,amount,
      mainCategory:cat?.main||"",subCategory:cat?.sub||"",section:cat?.section||"",
      remarks:"",isTransfer:cat?.section==="transfer",autoDetected:!!cat,balance:rawBalance,
    });
  }
...
export default function App() {
  ...
  function handleExport(){
    const data={transactions,categories,balances:accountBalances,merchantMemory,investments,exportDate:new Date().toISOString(),version:"v8"};
    const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`rathore-finance-${new Date().toISOString().split("T")[0]}.json`;a.click();URL.revokeObjectURL(url);
  ...
  function bulkReCategorize(description,main,sub,section){
    ...
    setTransactions(prev=>prev.map(t=>{
      if(t.description.toLowerCase().trim()===lower&&(t.mainCategory!==main||t.subCategory!==sub)){count++;return{...t,mainCategory:main,subCategory:sub,section,isTransfer:section==="transfer"};}
      return t;
    }));
  ...
  return (
    <div className="app-root">
      ...
      <div className="tab-content">
        ...
        {activeTab==="annual"       && <AnnualTab transactions={transactions} categories={categories} accountBalances={accountBalances} selectedYear={selectedYear} onPrevYear={()=>setSelectedYear(y=>y-1)} onNextYear={()=>setSelectedYear(y=>y+1)} onCategoriesChange={setCategories}/>}
      </div>
    </div>
  );
}
      ...
      <div className="card acc-card">
        ...
        {categories.income.map(h=>{
          ...
          return(<div key={h.head}>
            ...
            {headExp[k]&&h.subs.map(sub=>{const sB=h.budget[sub]||0,sA=sumActual("income",h.head,sub);return(<div key={sub} className="acc-item-row"><span className="acc-label-col acc-item-name">{sub}</span><span className="acc-num-col c-muted">{sB>0?fmt(sB):"—"}</span><span className={`acc-num-col ${colorCls(getActualColor(sA,sB,"income"))}`}>{sA>0?fmt(sA):"—"}</span></div>);})}
          </div>);
        })}
        ...
        {categories.expenses.map(h=>{
          ...
          return(<div key={h.head}>
            ...
            {headExp[k]&&h.subs.map(sub=>{const sB=h.budget[sub]||0,sA=counted.filter(t=>t.section==="expenses"&&t.mainCategory===h.head&&t.subCategory===sub).reduce((s,t)=>s+Math.abs(t.amount),0);return(<div key={sub} className="acc-item-row"><span className="acc-label-col acc-item-name">{sub}</span><span className="acc-num-col c-muted">{sB>0?fmt(sB):"—"}</span><span className={`acc-num-col ${colorCls(getActualColor(sA,sB,"expense"))}`}>{sA>0?fmt(sA):"—"}</span></div>);})}
          </div>);
        })}
        ...
        {savExp&&categories.savings.map(s=>{const sA=sumActual("savings",s.head);return(<div key={s.head} className="acc-item-row"><span className="acc-label-col acc-item-name">{s.head}</span><span className="acc-num-col c-muted">{s.budget>0?fmt(s.budget):"—"}</span><span className={`acc-num-col ${colorCls(getActualColor(sA,s.budget,"savings"))}`}>{sA>0?fmt(sA):"—"}</span></div>);})}
...
// ─── TRANSACTIONS TAB — SOURCE OF TRUTH ──────────────────────────────────────
...
function TransactionsTab({transactions,categories,merchantMemory,selectedMonth,onPrevMonth,onNextMonth,onImport,onUpdateTransaction,onDeleteTransaction,onClearMonth,onBulkReCategorize,onCategoriesChange,onUpdateMerchantMemory,showToast}){
  ...
  return(
    <div className="tab-pane">
      ...
      {showCatMgr&&<CategoryManager categories={categories} onChange={onCategoriesChange} onClose={()=>setShowCatMgr(false)} context="transactions"/>}
      ...
      <div className="card data-mgmt no-print">
        ...
        <div className="data-mgmt-row">
          ...
          >Clear {clearMonthSel?monthLabel(clearMonthSel):"selected month"}</button>
        </div>
      </div>
    </div>
  );
...
// ─── CATEGORY MANAGER ─────────────────────────────────────────────────────────
// context="transactions" → shows budget amounts (current month)
// context="annual"       → shows budget amounts (standard template label)

function CategoryManager({categories,onChange,onClose,context="transactions"}){
  ...
  const title   = context==="annual" ? "⚙ Standard Budget Template — applies to all future months" : "⚙ Categories & Budget — changes reflect in this month's Overview";
  const saveBtn = context==="annual" ? "Save Standard Template" : "Save Changes";

  return(
    <div className="cat-mgr card no-print">
      <div className="cat-mgr-hdr">
        <div><span className="cat-mgr-title">{title}</span></div>
...
function InvestmentsTab({investments,onInvestmentsChange,showToast}){
  ...
  function handleFile(e){
    const file=e.target.files[0];if(!file)return;
    const reader=new FileReader();
    reader.onload=ev=>{const rows=parseInvestmentCSV(ev.target.result,uploadAccount);onInvestmentsChange(prev=>[...prev.filter(r=>r.account!==uploadAccount),...rows]);showToast(`Imported ${rows.length} holdings for ${INVEST_ACCOUNTS.find(a=>a.id===uploadAccount)?.label}`);e.target.value="";};
  ...
  return(
    <div className="tab-pane">
      ...
      <div className="inv-cards">
        {INVEST_ACCOUNTS.map(acc=>{const rows=grouped[acc.id]||[],book=rows.reduce((s,r)=>s+r.bookValue,0),mkt=rows.reduce((s,r)=>s+r.marketValue,0),gain=mkt-book,pct=book>0?((gain/book)*100).toFixed(1):"0.0";return(<div key={acc.id} className="inv-card"><div className="inv-card-lbl">{acc.label}</div><div className={`inv-card-val ${mkt>0?"c-green":"c-muted"}`}>{mkt>0?fmt(mkt):"—"}</div>{mkt>0&&<div className="inv-card-sub">Book {fmt(book)} · <span className={gain>=0?"c-green":"c-red"}>{gain>=0?"+":""}{fmt(gain)} ({pct}%)</span></div>}</div>);})}
      </div>
...
// ─── ANNUAL TAB — STANDARD TEMPLATE + READ ONLY DATA ─────────────────────────

const MONTH_NAMES=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

function AnnualTab({transactions,categories,accountBalances,selectedYear,onPrevYear,onNextYear,onCategoriesChange}){
  ...
  return(
    <div className="tab-pane">
      ...
      {showTmpl&&<CategoryManager categories={categories} onChange={onCategoriesChange} onClose={()=>setShowTmpl(false)} context="annual"/>}
        ...
        <table className="tx-tbl annual-pivot">
          <thead>
            <tr>
              ...
              {MONTH_NAMES.map((mn,i)=>{const key=monthKeys[i],isCur=key===curMK,isFut=key>curMK&&!data[i].hasTx;return<th key={mn} className={`ann-month-hdr${isCur?" ann-cur-col":isFut?" ann-fut-col":""}`}>{mn}{isCur&&<span className="current-dot"> ●</span>}</th>;})}
          ...
          <tbody>
            {rows.map((row,ri)=>{
              ...
              return[
                ...
                <tr key={ri} className={`tx-row ann-data-row ${secCls}`}>
                  ...
                  {data.map((m,mi)=>{
                    const val=row.getValue(m),isCur=m.key===curMK,isFut=m.key>curMK&&!m.hasTx;
                    const hasVal=val!==null&&val!==undefined&&!isNaN(val)&&(m.hasTx||row.section==="balance");
                    return(<td key={mi} className={`ann-data-cell${isFut?" ann-fut-col":""}`}>{hasVal?<span className={`${row.bold?"ann-bold":""} ${isCur?"ann-cur-val":""}`}>{fmt(val)}</span>:<span className="c-muted">—</span>}</td>);
                  })}
            ...
            <tr className="tx-row ann-data-row ann-balance">
              ...
              {balanceChanges.map((change,i)=>{
                ...
                return(<td key={i} className={`ann-data-cell${isFut?" ann-fut-col":""}`}>{change!==null?<span style={{fontWeight:700,color:change>=0?"var(--green)":"var(--red)"}}>{change>=0?"+":""}{fmt(Math.abs(change))}</span>:<span className="c-muted">—</span>}</td>);
              })}
